...
def run_split_friedel(mtz, directory):
    ...
    friedel.split_friedel(parser)
    return pfile,mfile

def run_careless(pfile, mfile, directory, extra_flags=''):
    args = f" mono {extra_flags} --separate --iterations=10 --disable-gpu --double-wilson-r=0.,0.99 --double-wilson-parents=None,0 dHKL,image_id {pfile} {mfile} {directory}/out"
    parser = careless_parser.parse_args(args.split())
...
def test_split_friedel_and_run_careless(off_file):
    ...
    with TemporaryDirectory() as td:
        ...
        ds_p = rs.read_mtz(pfile)
        ds_m = rs.read_mtz(mfile)
...
def test_split_merge_and_combine(off_file):
    ...
    with TemporaryDirectory() as td:
        ...
        p_xval = p_merged.removesuffix('0.mtz') + 'xval_0.mtz'
        m_xval = m_merged.removesuffix('1.mtz') + 'xval_1.mtz'
        args = f" {p_xval} {m_xval} {combined}"
        parser = friedel.get_combine_friedel_parser()
        parser = parser.parse_args(args.split())
        friedel.combine_friedel(parser)

        assert exists(combined)
        ds = rs.read_mtz(combined)
        ds_p_xval = rs.read_mtz(p_xval)
        ds_m_xval = rs.read_mtz(m_xval)
