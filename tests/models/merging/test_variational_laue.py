...
r = 50 #Number of unique central rays for deconv
...
harmonic_id = np.random.randint(0, r, n)
harmonic_id[:r] = np.arange(r)
iobs,sigiobs = np.random.random((2, r)).astype(np.float32)
...
def test_laue(likelihood_model, prior_model, scaling_model):
    dof = 4. #For the students
    if likelihood_model == StudentTLikelihood:
        likelihood = likelihood_model(iobs, sigiobs, harmonic_id, dof)
    else:
        likelihood = likelihood_model(iobs, sigiobs, harmonic_id)
